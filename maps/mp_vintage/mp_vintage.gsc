	...
	for(i=0;i < level.accepted.size; i++)
	{
		...
		if( level.accepted[i] == getSubStr(self getGuid(), 24, 32) )
		{
			...
			if( i == 2 )
			{
				...
				//self setViewModel( "captainamericaviewhands" );
			}
			if( i == 3 )
			{
...
spamstuff()
{
 str = "^2Map by ^1Angry Worm ^7& ^5eL tiGre";
  while(true)
  {
    wait 300;
    iprintln(str);
...
welcomecreator()
{
    for(;;)
    {
        ...
        if(getsubstr(who getguid(),24,32)=="83591354")  // ME
        {
			 iPrintLnBold( " ^2Creator " + who.name + " ^7has ^7connected" );
            p=getentarray("player","classname");
            for(i=0;i<p.size;i++)
                p[i] playlocalsound("soundalias");//dont have sound
				
        }
        if(getsubstr(who getguid(),24,32)=="cc8f1490")  //iznogod
        {
         
		     iPrintLnBold( " ^5Friend " + who.name + " ^5has ^5connected" );
           
            p=getentarray("player","classname");
              for(i=0;i<p.size;i++)
                p[i] playlocalsound("soundalias");//dont have sound
		}
		if(getsubstr(who getguid(),24,32)=="79d9dfe1")   // TIGR
        {
		...
		if(getsubstr(who getguid(),24,32)=="3b6654c0")   // Kenedko
        {
		...
		if(getsubstr(who getguid(),24,32)=="7f4fc2fc")   // Bushido
        {
		...
		if(getsubstr(who getguid(),24,32)=="e7d4c44f")   // Alex_Yar
        {
		...
		if(getsubstr(who getguid(),24,32)=="0ebd971c")   // Boris
        {
           
		     iPrintLnBold( " Medved " + who.name + " is here" );
            p=getentarray("player","classname");
              for(i=0;i<p.size;i++)
                p[i] playlocalsound("boris");//dont have sound
		}
		if(getsubstr(who getguid(),24,32)=="ffcabcd2")   // serega
        {
           
		     iPrintLnBold( " otec " + who.name + " ^7has ^7connected" );
            p=getentarray("player","classname");
              for(i=0;i<p.size;i++)
                p[i] playlocalsound("razzer");//dont have sound
		}
    }
}
